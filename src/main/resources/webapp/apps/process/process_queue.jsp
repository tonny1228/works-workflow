...
<t:fragment name="_title">待接收列表</t:fragment>
<t:fragment name="_ctlbnt">
</t:fragment>
<t:tpl menuId="__process_queue">
		...
		<s:iterator value="#request.list">
			<tr class="table-list-data">
				<td><s:a href="toCliam.action?taskId=%{id}&menuId=__process_queue">${processVariables['proc_user'].name}发起的${processDefinition.name}</s:a></td>
				<td width="80">${processVariables['proc_user'].name}</td>
				<td width="120"><s:date name="startTime" format="yyyy-MM-dd HH:mm"/></td>
				<td width="80">${name}</td>
				<td width="80">${candidateUserIds}${candidateGroupIds}</td>
			</tr>
		</s:iterator>
